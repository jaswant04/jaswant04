## Hi, I'm Jaswant 👋

💻 Backend-focused Software Developer, primarily working with Python-based systems, and currently exploring the AI/ML space 🤖

### Current Focus

- ⚡ Building and maintaining backend systems
- 📚 Strengthening Python fundamentals for scalable services
- 🤖 Exploring AI/ML concepts and workflows

### Tech Stack

- **Languages:** 🐍 Python, ☕ Java  
- **Backend / APIs:** ⚡ FastAPI, RESTful APIs, Spring Boot  
- **Databases:** 🗄️ MySQL  
- **AI / ML:** 🤖 Machine Learning fundamentals, model development (classification, regression)  
- **Cloud / Messaging:** ☁️ AWS Lambda, Amazon SQS, CloudWatch 

### 📫 Connect with me

[<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" width="28"/>](https://www.linkedin.com/in/jaswant-yadav04/)
[<img src="https://upload.wikimedia.org/wikipedia/commons/4/4e/Gmail_Icon.png" width="28"/>](mailto:yadavjas01@gmail.com)

# 📊 GitHub Stats:
[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=jaswant04&show_icons=true&theme=dark&count_private=true)](https://github.com/jaswant04)
